...
class Fingerprint(object):
    ...
    """
    def __init__(self, Rc1=6.0, Rc2=4.0, binwidth1=0.2, Nbins2=30, sigma1=0.2, sigma2=0.2, eta=20, gamma=2, use_angular=True, use_fixed_atoms=True):
        ...
        self.use_fixed_atoms = use_fixed_atoms
    ...
    def initialize_from_atoms(self, atoms):
        ...
        # Determine indices of atoms, for which the radial and angular
        # neighbourhoods contribute to the descriptor.
        self.indices_of_core_atoms = np.arange(self.Natoms)
        self.N_core_atoms = atoms.get_number_of_atoms()
        if not self.use_fixed_atoms:
            for constraint in atoms.constraints:
                if isinstance(constraint, FixAtoms):
                    indices_fixed = constraint.get_indices()
                    self.indices_of_core_atoms = np.delete(np.arange(self.Natoms), indices_fixed)
                    self.N_core_atoms = len(self.indices_of_core_atoms)
    ...
    def get_feature(self, atoms):
        ...
        # set up core atoms array
        cdef int N_core_atoms = self.N_core_atoms
        cdef int *indices_of_core_atoms
        indices_of_core_atoms = <int*>mem.alloc(N_core_atoms, sizeof(int))
        for idx_core in range(N_core_atoms):
            indices_of_core_atoms[idx_core] = self.indices_of_core_atoms[idx_core]
        ...
        cdef double Rij, normalization, binpos, erf_const, erfarg_low, erfarg_up, value
        cdef int i, i0, j, n

        erf_const = 1./sqrt(2)*binwidth1/sigma1
        for i0 in range(N_core_atoms):
            i = indices_of_core_atoms[i0]
            for cell_index in range(Ncells):
                ...
                for j in range(Natoms):
                    ...
                    for n in range(minbin_lim, maxbin_lim + 1):
                        ...
                        value = erf(erf_const*erfarg_up)-erf(erf_const*erfarg_low)
        ...
        cdef double angle, erf_const2, cutoff_scaling, scaling
        cdef int k, cond_ij, cond_ik, type3
        erf_const2 = 1./sqrt(2)*binwidth2/sigma2
        for i0 in range(N_core_atoms):
            i = indices_of_core_atoms[i0]
            pos_i = pos[i]
                ...
                for j in range(Natoms):
                    ...
                    for cell_index2 in range(cell_index1, Ncells):
                        ...
                        for k in range(k_start, Natoms):
                            ...
                            # Evaluate cutoff scaling
                            cutoff_scaling = f_cutoff(Rij, gamma, Rc2) * f_cutoff(Rik, gamma, Rc2)
                            ...
                            scaling = cutoff_scaling*normalization
                            ...
                            for n in range(minbin_lim, maxbin_lim + 1):
                                ...
                                value = erf(erf_const2*erfarg_up)-erf(erf_const2*erfarg_low)
                                # Apply scaling
                                value *= scaling
    ...
    def get_featureGradient(self, atoms):
        ...
        cdef Point RijVec
        cdef int num_pairs, center_bin, minbin_lim, maxbin_lim, newbin, bondtype_index, type1, type2
        cdef double Rij, normalization, binpos, erf_const, arg_low, arg_up, value1, value2, value, Rij_inv, front_const
        cdef int i, i0, j, n
        
        erf_const = 1./sqrt(2)*binwidth1/sigma1
        front_const = -1./(sigma1*sqrt(2*M_PI))
        for i0 in range(N_core_atoms):
            i = indices_of_core_atoms[i0]
            pos_i = pos[i]
            for cell_index in range(Ncells):
                ...
                for j in range(Natoms):
                    ...
                    for n in range(minbin_lim, maxbin_lim + 1):
                        ...
                        value1 = -Rij_inv * (erf(erf_const*arg_up) - erf(erf_const*arg_low))
                        value2 = front_const * (exp(-pow(erf_const*arg_up,2)) - exp(-pow(erf_const*arg_low,2)))
                        value = value1 + value2
                        ...
                        for m in range(3):
                            feature_grad1[(bondtype_index + newbin) * Natoms*dim + dim*i+m] += -value*Rij_inv * RijVec.coord[m]
                            feature_grad1[(bondtype_index + newbin) * Natoms*dim + dim*j+m] += value*Rij_inv * RijVec.coord[m]
        ...
        cdef double angle, cos_angle, a, erf_const2, erf_const2_pow2, front_const2, const_ik_ijgrad, const_ij_ikgrad, const_ijk, Rik_inv
        cdef int k, cond_ij, cond_ik, bin_index, type3
        
        erf_const2 = 1./sqrt(2)*binwidth2/sigma2
        erf_const2_pow2 = erf_const2*erf_const2
        front_const2 = -1./(sigma2*sqrt(2*M_PI))
        for i0 in range(N_core_atoms):
            i = indices_of_core_atoms[i0]
            pos_i = pos[i]
            for cell_index1 in range(Ncells):
                    ...
                    for cell_index2 in range(cell_index1, Ncells):
                        ...
                        for k in range(k_start, Natoms):
                            ...
                            for n in range(minbin_lim, maxbin_lim + 1):
                                ...
                                value1 = 0.5*(erf(erf_const2*arg_up)-erf(erf_const2*arg_low))
                                value2 = front_const2 * (exp(-erf_const2_pow2*arg_up*arg_up) - exp(-erf_const2_pow2*arg_low*arg_low))

                                # Apply normalization
                                value1 *= normalization
                                value2 *= normalization

                                const_ik_ijgrad = value1 * fc_ik * fc_grad_ij * Rij_inv
                                const_ij_ikgrad = value1 *fc_ij * fc_grad_ik * Rik_inv
                                const_ijk = value2 *fc_ij * fc_ik

                                bin_index = (bondtype_index + newbin) * Natoms*dim
                                for m in range(3):
                                    feature_grad2[bin_index + dim*i+m] += -const_ik_ijgrad * RijVec.coord[m]
                                    feature_grad2[bin_index + dim*j+m] += const_ik_ijgrad * RijVec.coord[m]

                                    feature_grad2[bin_index + dim*i+m] += -const_ij_ikgrad * RikVec.coord[m]
                                    feature_grad2[bin_index + dim*k+m] += const_ij_ikgrad * RikVec.coord[m]

                                    feature_grad2[bin_index + dim*i+m] += const_ijk * angle_grad_i.coord[m]
                                    feature_grad2[bin_index + dim*j+m] += const_ijk * angle_grad_j.coord[m]
                                    feature_grad2[bin_index + dim*k+m] += const_ijk * angle_grad_k.coord[m]
